...
func New(repo *repository.Repository, cartClient cart.ShoppingCartServiceClient, paymentFailedWriter, paymentSucceedWriter *kafka.Writer) *Service {
	return &Service{
		repo:                 repo,
...
func (s *Service) ConfirmOrderPayment(ctx context.Context, eventData OrderData) error {
	...
	orderAmountInCents := int64(math.Round(eventData.Amount * 100))
